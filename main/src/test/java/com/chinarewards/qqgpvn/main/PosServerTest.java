 ...
 * @since 0.1.0
 */
public class PosServerTest extends GuiceTest {
	...
	protected Module[] getModules() {
		Module[] modules = new Module[] { buildTestConfigModule(),
				new ServerModule() };
	...
	protected Module buildTestConfigModule() {
		...
		// persistence
		conf.setProperty("db.user", 1235);
		conf.setProperty("db.password", 1235);
		conf.setProperty("db.dialect", 1235);
		conf.setProperty("db.driver", 1235);
		conf.setProperty("db.url", 1235);
		// additional Hibernate properties
		conf.setProperty("db.hibernate.dialect", "org.hibernate.dialect.HSQLDialect");
		conf.setProperty("db.hibernate.show_sql", true);
		// URL for QQ
		conf.setProperty("qq.groupbuy.url.groupBuyingSearchGroupon", "http://localhost:8086/qqapi");
		conf.setProperty("qq.groupbuy.url.groupBuyingValidationUrl", "http://localhost:8086/qqapi");
		conf.setProperty("qq.groupbuy.url.groupBuyingUnbindPosUrl", "http://localhost:8086/qqapi");
