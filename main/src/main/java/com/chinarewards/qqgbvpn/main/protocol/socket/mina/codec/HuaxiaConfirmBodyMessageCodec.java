...
public class HuaxiaConfirmBodyMessageCodec implements ICommandCodec {
	...
	public ICommand decode(IoBuffer in, Charset charset)
		...
		log.debug("HuaxiaConfirm request message decode");
		HuaxiaRequestMessage message = new HuaxiaRequestMessage();
		if (in.remaining() < ProtocolLengths.COMMAND) {
			throw new PackageException(
					"HuaxiaConfirm packge message body error, body message is :"
							+ in);
		}
		...
		if (errorFlag) {
			throw new PackageException(
					"HuaxiaConfirm packge message body error, body message");
		}
		String cardNum = null;
		if (cardNumEnd != 0) {
		...
		log.debug("HuaxiaConfirm message request:cmdId is ({}) , cardNum is ({})"
