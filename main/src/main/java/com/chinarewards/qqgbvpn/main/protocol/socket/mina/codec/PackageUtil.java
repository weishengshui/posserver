...
public final class PackageUtil {
	...
	protected static Logger logger = LoggerFactory.getLogger(PackageUtil.class);
	 ...
	 * @author Seek
	 */
	public static final byte[] formatHeadContent(long currentSequence, byte[] content) 
		...
		try{
			...
			headMessage.setMessageSize(ProtocolLengths.HEAD + content.length);
			...
			PackageHeadCodec packageHeadCodec = new PackageHeadCodec();
			byte[] heads = packageHeadCodec.encode(headMessage);
			
			logger.debug("heads bytes:"+Arrays.toString(heads) + ", length="+heads.length);
			logger.debug("bodys bytes:"+Arrays.toString(content) + ", length="+content.length);
