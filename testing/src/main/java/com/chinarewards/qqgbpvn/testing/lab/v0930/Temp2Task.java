 ...
 * @author Seek
 */
public class Temp2Task extends PosTask {
	...
	private Message sendMessage(JavaSamplerContext context) throws SendMessageException {
		try{
			//build get list body Message
			Map<String, String> map = new HashMap<String, String>();
			BuildMessage getGroupBuyListBuildMessage = MessageFactory.getBuildMessage(
					BusinessType.PosGetQQGroupBuyList);
			byte[] foreBodys = getGroupBuyListBuildMessage.buildBodyMessage(map);
			
			//format package content
			byte[] requestBytes1 = formatPackageContent(context, foreBodys);
			...
			byte[] proxyRequestBytes = new byte[requestBytes1.length + requestBytes2.length];
			//copy bytes to bytes
			System.arraycopy(requestBytes1, 0, proxyRequestBytes, 0, requestBytes1.length);
			System.arraycopy(requestBytes2, 0, proxyRequestBytes, requestBytes1.length, requestBytes2.length);
			
			SocketUtil.writeFromOutputStream(TestContext.getBasePosConfig().getSocket().
					getOutputStream(), proxyRequestBytes);
